...
mod tests {
    ...
    #[case::two(Box::new(|ser: &mut OS| { ser.write_int_value("a", 1)?; ser.write_int_value("b", 2) }), r#"{"a":1,"b":2}"#)]
    #[case::nested_arr(Box::new(|ser: &mut OS| { ser.start_array("x")?.end() }), r#"{"x":[]}"#)]
    #[case::nested_arr_with_el(Box::new(|ser: &mut OS| { let mut n = ser.start_array("y")?; n.write_int_value(5) }), r#"{"y":[5]}"#)]
    #[case::nested_arr_first(Box::new(|ser: &mut OS| { ser.start_array("z")?.end()?; ser.write_int_value("q", 4) }), r#"{"z":[],"q":4}"#)]
    #[case::nested_arr_last(Box::new(|ser: &mut OS| { ser.write_int_value("q", 6)?; ser.start_array("z")?.end() }), r#"{"q":6,"z":[]}"#)]
    #[case::nested_arr_between(Box::new(|ser: &mut OS| { ser.write_int_value("a", 7)?; ser.start_array("b")?.end()?; ser.write_int_value("c", 9) }), r#"{"a":7,"b":[],"c":9}"#)]
    ...
    #[case::nested_obj_with_el(Box::new(|ser: &mut OS| { let mut n=ser.start_object("g")?; n.write_int_value("a", 3) }), r#"{"g":{"a":3}}"#)]
    #[case::nested_obj_first(Box::new(|ser: &mut OS| { ser.start_object("h")?.end()?; ser.write_int_value("i", 0) }), r#"{"h":{},"i":0}"#)]
    #[case::nested_obj_last(Box::new(|ser: &mut OS| { ser.write_int_value("j", 2)?; ser.start_object("k")?.end() }), r#"{"j":2,"k":{}}"#)]
    ...
    #[case::u8(Box::new(|w: &mut OS| w.write_int_value("a", 2u8)), "2")]
    #[case::i8(Box::new(|w: &mut OS| w.write_int_value("a", -3i8)), "-3")]
    #[case::u16(Box::new(|w: &mut OS| w.write_int_value("a", 4u16)), "4")]
    #[case::i16(Box::new(|w: &mut OS| w.write_int_value("a", -5i16)), "-5")]
    #[case::u32(Box::new(|w: &mut OS| w.write_int_value("a", 6u32)), "6")]
    #[case::i32(Box::new(|w: &mut OS| w.write_int_value("a", -7i32)), "-7")]
    #[case::u64(Box::new(|w: &mut OS| w.write_int_value("a", 8u64)), "8")]
    #[case::i64(Box::new(|w: &mut OS| w.write_int_value("a", -9i64)), "-9")]
    ...
    #[case::i128(Box::new(|w: &mut OS| w.write_int_value("a", -13i128)), "-13")]
    #[case::f64(Box::new(|w: &mut OS| w.write_f64_value("a", 2.0)), "2")]
    #[case::f64_exp(Box::new(|w: &mut OS| w.write_f64_value("a", 1.234e10)), "12340000000")]
    #[case::f64_exp_20(Box::new(|w: &mut OS| w.write_f64_value("a", 1.234e20)), "123400000000000000000")]
    #[case::f64_neg_exp(Box::new(|w: &mut OS| w.write_f64_value("a", 1.234e-10)), "0.0000000001234")]
    ...
    #[case::f32_exp(Box::new(|w: &mut OS| w.write_f32_value("a", 1.234e10)), "12340000000")]
    #[case::f32_exp_20(Box::new(|w: &mut OS| w.write_f32_value("a", 1.234e20)), "123400000000000000000")]
    #[case::f32_neg_exp(Box::new(|w: &mut OS| w.write_f32_value("a", 1.234e-10)), "0.0000000001234")]
    ...
    fn test_write_value(#[case] code: Box<dyn Fn(&mut OS) -> io::Result<()>>, #[case] expected: &str) -> io::Result<()> {
        ...
        // test with and without preceding element to verify that 'initial' is handled correctly
        {
            let mut writer = JsonWriter::new_compact(Vec::new());
            {
                ...
                code(&mut object_ser)?;
                object_ser.write_int_value("y", 5)?;
            }
